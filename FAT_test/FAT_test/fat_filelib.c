...
int fl_attach_media(fn_diskio_read rd, fn_diskio_write wr)
{
	...
	//fl_format(100000, "test_format"); //uncomment if needs formatting
    // Initialise FAT parameters
    if ((res = fatfs_init(&_fs)) != FAT_INIT_OK)
    {
#ifdef _debug_
        USART_printf("FAT_FS: Error could not load FAT details (%d)!\r\n", res);
#endif
        return res;
    }
...
void fl_listdirectory(const char *path)
{
	...
	#ifdef _debug_
            USART_printf("%02d/%02d/%04d  %02d:%02d      ", d,mn,y,h,m);
	#endif
#endif
#ifdef _debug_
            if (dirent.is_dir)
